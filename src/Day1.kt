fun main() {
    println(numTable)
     ...
     val sum = splitLines(input)
         .map { spelledToNumber2(it) }
         .map { stripAlpha(it) }
         .map { duplicateSingleChar(it) }
         .map { takeFirstAndLast(it) }
         .sumOf { convertToDecimal(it) }
...
fun splitLines(input: String): List<String> = input.lines()
fun spelledToNumber(input: String): String {
    val validSpellings = listOf(
        "oneight" to "18",
        "twone" to "21",
        "eightwo" to "82",
        "eightree" to "83",
    ...
    return validSpellings.fold(input) { acc, pair ->
        acc.replace(pair.first, pair.second)
    }
}
val numTable = createNumTable()
fun spelledToNumber2(input: String): String {
    return numTable.fold(input) { acc, pair ->
        acc.replace(pair.first, pair.second)
    }
}
fun stripAlpha(input: String): String = input.filter { it.isDigit() }
fun duplicateSingleChar(input: String): String = if (input.length == 1) input + input else input
fun takeFirstAndLast(input: String): String = input.take(1) + input.takeLast(1)
fun convertToDecimal(input: String): Int = input.toInt()


fun createNumTable(): List<Pair<String, String>> {
    val validSpellings = listOf(
        "one" to "1",
        "two" to "2",
        "three" to "3",
    ...
    fun lastLetterMatchesFirstLetter(that: Pair<String, String>): List<Pair<String, String>> {
        val found = validSpellings.filter { it.first.first() == that.first.last() }
        return found.map { that.first.dropLast(1) + it.first to that.second + it.second }
    }

    val result = validSpellings.fold(validSpellings) { acc, pair ->
        lastLetterMatchesFirstLetter(pair) + acc
    }
